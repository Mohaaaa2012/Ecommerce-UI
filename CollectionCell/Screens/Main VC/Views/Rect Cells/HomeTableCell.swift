...
class HomeTableCell: UITableViewCell {
    ...
    static let cellId = String(describing: HomeTableCell.self)
    
    var cellHeight: CGFloat = 0
    var cellWidth: CGFloat = 0
    var cellsNumber: Int = 0
    let images = ["basics", "entertain", "furniture", "toys", "tools"]
    let labels = ["KITCHEN BASICS", "ENTERTAINMENT", "FURNITURE", "TOYS", "TOOLS"]
    
    var HomeSectionDelegate: pushControllers?
    
    
    let HomeCollectionView = UICollectionView(
        frame: .zero,
        collectionViewLayout: HomeTableCell.createLayout())
    ...
    static func createLayout() -> UICollectionViewCompositionalLayout {
       ...
       let compoundhalfGroup = NSCollectionLayoutGroup.horizontal(layoutSize: compoundGroupSize, subitem: secondItem, count: 2)
    ...
    func setupCollectionView() {
        ...
        HomeCollectionView.backgroundColor = UIColor(named: "headerColor")
        HomeCollectionView.showsHorizontalScrollIndicator = false
        HomeCollectionView.delegate = self
        HomeCollectionView.dataSource = self
        HomeCollectionView.register(SmallRectCollectionCell.self, forCellWithReuseIdentifier: SmallRectCollectionCell.cellId)
        HomeCollectionView.setAnchor(top: topAnchor, leading: leadingAnchor, trailing: trailingAnchor, bottom: bottomAnchor, paddingTop: 0, paddingLeading: 5, paddingTrailing: 5, paddingBottm: 5)
    }
}
...
extension HomeTableCell: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        HomeSectionDelegate?.pushFromHomeSection(index: indexPath)
    }
